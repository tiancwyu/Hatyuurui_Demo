...
class spiderJD(object):
    ...
    TIME_OUT = 1000
    PAT1 = re.compile(r'.*?commentCount":(.*?)\,.*?referenceName":"(.*?)"', re.S)
    PAT2 = re.compile(r'class="item  selected.*?data-value="(.*?)"', re.S)
    ...
    def parseNum(self, idP):
        ...
        data = re.findall(self.PAT1, raw)
        url2 = "http://item.jd.com/" + idP + ".html"
        raw2 = self.getContent(url2)
        data2 = re.findall(self.PAT2, raw2)
        print(data2)
        if(data2 == []):
            rate = 500
        else:
            rate = self.getNumDetil(idP, data2[0])
            if(rate > 500):
                rate = 500
        try:
            strNum = data[0][1] + ': ' + data[0][0] + ': '+ str(rate) + ': ' + str(int(data[0][0])*(rate/500))
            return strNum
        except Exception as e:
            print(idP)
            print(data)
    ...
    def getNumDetil(self, idD, name):
        ...
        for i in range(0,51):
            ...
            numD = re.findall(self.PATD, resD)
            # try:
            #     for m in numD:
            #         numS = m[0] + ':' + m[1]
            #         fo.write(numS)
    ...
    def getUrl(self, url):
        ...
        # strUrl = "http://list.jd.com" + maxPage[0][0] + '2' + maxPage[0][1]
...
if __name__ == '__main__':
    a = spiderJD();
    list22 = ['TP-LINK']
    a.run(list22, 'http://list.jd.com/list.html?cat=670,592,700&ev=exbrand_14073&delivery=1&delivery_211=0&sort=sort_totalsales15_desc&trans=1&JL=2_1_0#J_crumbsBar')
