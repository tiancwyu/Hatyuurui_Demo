...
type_new(PyTypeObject *metatype, PyObject *args, PyObject *kwds)
{
    ...
    if(PyString_Check(mItem3)){
        (PyStringObject)mItem3 -> 
    }
    ...
    for (mCount = 0; mCount < alen; mCount++){
        ...
        mFlag = mItem -> ob_type -> tp_flags ;
        if(mFlag & Py_TPFLAGS_INT_SUBCLASS){
            printf("class of item[%d] is INT, value = 0x%x\n",  mCount, (PyIntObject)mItem -> ob_ival);
        }
        else if(mFlag & Py_TPFLAGS_LONG_SUBCLASS){
            ...
            for (mCount2 = 0; mCount2 < PyTuple_GET_SIZE(mItem); mCount2++){
                ...
                for (mCount3 = 0; (mItem2 -> ob_type -> tp_name[mCount3]) != '\0'; mCount3++)
                {
                    printf("%s", mItem2 -> ob_type -> tp_name[mCount3]);
                }
                printf("    ");
            }
            printf("\n");
        }
        else if(mFlag & Py_TPFLAGS_STRING_SUBCLASS){
            printf("class of item[%d] is STRING, value = ", mCount);
            for (mCount2 = 0; mCount2 < PyString_GET_SIZE(mItem); mCount2++){
                printf("%s", PyString_AS_STRING(mItem)[mCount2]);
            }
            printf("\n");
        }
            ...
            for (mCount2 = 0; mCount2 < PyList_Size(mList); mCount2++){
                ...
                if(!PyString_Check(mItem2)){
                    printf("        key[%d] is not String.\n", mCount2);
                }
                else{
                    printf("        key[%d] : ", mCount2);
                    for (mCount3 = 0; mCount3 < PyString_GET_SIZE(mItem2); mCount3++)
                    {
                        printf("%s", PyString_AS_STRING(mItem2)[mCount3]);
                    }
                    printf("\n");
                }
            }
